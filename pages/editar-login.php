    ...
    if (isset($_POST['acao'])) {
      ...
      $password = $_POST['password'] ?? $login[0]['password'];
        ...
        if ($sql->rowCount() == 1) {
          ...
          Você será redirecionado para a página de edição de login em alguns segundos.
        </div>';
          ob_flush();
          flush();
          sleep(3);
      ...
      ?>
        ...
        <button name="acao" type="submit" class="btn btn-primary w-100">Atualizar Login</button>
      </form>
    </div>
  </div>
</div>